...
// Component Imports
import ProductAddHeader from '@/views/mineralwater/machinery/add/ProductAddHeader'
import ProductInformation from '@/views/mineralwater/machinery/add/ProductInformation'
import ProductImage from '@/views/mineralwater/machinery/add/ProductImage'
import { useRouter } from 'next/navigation'
import { InventoryDataParams, MachineryDataParams } from '@/types/apps/ecommerceTypes'
...
import { createMachineryList } from '@/libs/actions/machinery.actions'

const machineryDetailSchema = z
  .object({
    ...
    pricePerOne: z.coerce.number().min(1, { message: "Price Per Machine is required" }),
    machineryCategory: z.string().min(1, { message: "Machinery Category is required" }),
...
const formSchema = z.object({
  details: z.array(machineryDetailSchema).min(1, { message: "At least one machinery item is required" }),
});
...
const ProductDetails: React.FC = () => {
  ...
  const { control, handleSubmit, formState: { errors }, setValue } = useForm<MachineryDataParams>({
  ...
  async function onSubmit(data: MachineryDataParams) {
    setLoading(true)
    try {
      console.log(data)
      const res = await createMachineryList(data, user.user.$id!);
      ...
      console.error(error)
      toast.error(error instanceof Error? error.message : 'Failed to process machinery')
      setLoading(false)
  ...
  const onError = (errors: FieldErrors<MachineryDataParams>) => {
  ...
  const handleCategorySelect = (index: number) => async (data: any) => {
    ...
    setValue(`details.${index}.pricePerOne`, data.pricePerOne)
  ...
  return (
    <Card elevation={0}>
      <CardHeader title='Machine/Tool Details' />
      <CardContent>
              ...
              <Grid container spacing={4}>
                <Grid item xs={12} sm={paymentStatuses[index] !== 'credit'? 4 : 6}>
                  <ProductCategory
                    control={control}
                    index={index}
                    name={`details.${index}.machineryCategory`}
                    error={errors.details?.[index]?.machineryCategory}
                ...
                <Grid item xs={12}>
                  <ProductInformation
                    ...
                    names={{
                      ...
                      pricePerOne: `details.${index}.pricePerOne`,
              ...
              <Grid container spacing={6}>
                <Grid item xs={12}>
                  <ProductImage
                    control={control}
                    index={index}
                    name={`details.${index}.machineryImages`}
                  />
                </Grid>
              </Grid>
            </Grid>
