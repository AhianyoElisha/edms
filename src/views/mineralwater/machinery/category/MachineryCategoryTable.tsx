...
import { getMachineryCategoryList } from '@/libs/actions/machinery.actions'
...
type CategoryWithActionsType = MachineryCategoryType & {
  actions?: string
}
...
const MachineryCategoryTable = () => {
  // States
  const [addCategoryOpen, setAddCategoryOpen] = useState(false)
  const [editCategoryOpen, setEditCategoryOpen] = useState<{ open: boolean; data: MachineryCategoryType | null }>({ open: false, data: null })
  const [rowSelection, setRowSelection] = useState({})
  const [data, setData] = useState<MachineryCategoryType[]>([])
  ...
  const fetchMachineryData = useCallback(async () => {
    try {
      setIsLoading(true)
      const response = await getMachineryCategoryList(false, false)
      setData(response?.machineryList?.documents as unknown as MachineryCategoryType[])
    } catch (error) {
      console.error('Error fetching machinery requisition data:', error)
  ...
  const columns = useMemo<ColumnDef<CategoryWithActionsType, any>[]>(
    () => [
      ...
      columnHelper.accessor('pricePerOne', {
        header: 'Price/Machine GH₵',
        cell: ({ row }) => (
          <Typography>
            {
              // @ts-ignore
              row.original.pricePerOne
            }
          </Typography>
        )
      }),
  ...
  return (
    <>
      <Typography className='mt-4' variant='h4'>Machinery/Tools Categories</Typography>
...
export default MachineryCategoryTable
