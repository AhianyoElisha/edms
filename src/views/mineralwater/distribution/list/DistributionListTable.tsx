...
import type { DistributedItemDetailType, InventoryListType, ManufacturedProductListType, ProductDetailsType, ProductionCategoryType } from '@/types/apps/ecommerceTypes'
...
import { getDistributionProductList } from '@/libs/actions/distribution.actions'
...
type DistributionWithActionsType = DistributedItemDetailType & {
  actions?: string
}
...
// Column Definitions
const columnHelper = createColumnHelper<DistributionWithActionsType>()

const DistributionListTable = () => {

  // States
  const [rowSelection, setRowSelection] = useState({})
  const [data, setData] = useState<DistributedItemDetailType[]>([])
  ...
  const fetchInventoryData = useCallback(async () => {
    try {
      setIsLoading(true)
      const response = await getDistributionProductList()
      setData(response?.documents as unknown as DistributedItemDetailType[])
  ...
  const columns = useMemo<ColumnDef<DistributionWithActionsType, any>[]>(
    () => [
      ...
      columnHelper.accessor('$createdAt', {
        ...
        cell: ({ row }) => {
          const dateString = row.original.distributionDate!;
          let formattedDate;
      ...
      columnHelper.accessor('vehicle', {
        ...
        cell: ({ row }) => <Typography>{
        // @ts-ignore
          row.original.vehicles.vehicleType === 'truck' ? <i className="ri-truck-line text-[16px]"></i> : <i className="ri-e-bike-line text-[16px]"></i>} {`${row.original.vehicles.vehicleNumber}`}</Typography>
      }),
  ...
  const table = useReactTable({
    data: filteredData as DistributedItemDetailType[],
    columns,
    filterFns: {
      fuzzy: fuzzyFilter
    },
  ...
  return (
    <>
      <Typography className='mt-4' variant='h3'>Distributed Products Report</Typography>
