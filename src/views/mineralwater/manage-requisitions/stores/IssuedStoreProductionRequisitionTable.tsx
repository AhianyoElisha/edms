...
import { issueStoreRequisition, getIssuedStoreRequisitionList } from '@/libs/actions/stores.actions'
...
const StoreProductionRequisitionTable = () => {
  ...
  const fetchInventoryData = useCallback(async () => {
    try {
      setIsLoading(true)
      const response = await getIssuedStoreRequisitionList()
  ...
  const handleBulkAction = async (action: 'issue' | 'deny') => {
    try {
      setIsLoading(true)
      ...
      for (const row of selectedRows) {
        if (action === 'issue') {
          const res = await issueStoreRequisition(row.$id!, row.category.$id, user.user.$id)
          // @ts-ignore
        //  if (res?.code != 200) throw new Error(res?.message)
        } 
      }
      
      toast.success(`Successfully ${action}d ${selectedRows.length} approved requisitions`)
      setRowSelection({})
  ...
  const handleRequisitionAction = async (
    ...
    try {
      setIsLoading(true)
      if (action === 'issue') {
        await issueStoreRequisition(requisitionId, categoryId, user.user.$id)
              ...
              options={[
                {
                  ...
                  menuItemProps: {
                    onClick: () => handleRequisitionAction('issue', row.original.$id!, row.original.category.$id)
                  }
                },
              ]}
            />
  ...
  return (
    <>
      <Typography className='mt-4' variant='h4'>Issue Approved Items to Production</Typography>
        ...
        <div className='flex justify-between flex-col items-start flex-wrap sm:flex-row sm:items-center gap-4 p-5'>
          {isAdmin && selectedRowCount > 0 && (
            <div className='flex gap-4'>
              ...
              >
                Issue Selected ({selectedRowCount})
              </Button>
            </div>
          )}
        </div>
