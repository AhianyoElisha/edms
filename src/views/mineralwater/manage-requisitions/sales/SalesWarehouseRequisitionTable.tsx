...
// Type Imports
import type { CategoryType, InventoryListType, ProductionCategoryType } from '@/types/apps/ecommerceTypes'
...
interface ProductionStoreRequisitionTableProps {
...
type ProductionCategoryWithActionsType = ProductionCategoryType & {
  actions?: string
}
...
const WarehouseSalesRequisitionTable = ({ refreshTrigger, onSuccessfulSubmit }: ProductionStoreRequisitionTableProps) => {
  // States
  const [status, setStatus] = useState<InventoryListType['status']>('All')
  const [rowSelection, setRowSelection] = useState({})
  const [allData, setAllData] = useState<ProductionCategoryType[]>([])
  ...
  const fetchInventoryData = useCallback(async () => {
    try {
      ...
      setAllData(data?.productionList.documents as unknown as ProductionCategoryType[])
    } catch (error) {
  ...
  const columns = useMemo<ColumnDef<ProductionCategoryWithActionsType, any>[]>(
    () => [
      ...
      //   )
      // }),
      columnHelper.accessor('warehouseRequisitionRequest', {
        header: 'Pending Requisition',
        cell: ({ row }) => (
          <Typography>
            {row.original.warehouseRequisitionRequest}
          </Typography>
        )
      }),
      columnHelper.accessor('totalWarehouseProducts', {
        header: 'Total Products',
        cell: ({ row }) => <Typography>{row.original.totalWarehouseProducts}</Typography>
      }),
      columnHelper.accessor('warehouseStatus', {
        header: 'Status',
        cell: ({ row }) => (
          <div className='flex items-center gap-3'>
            <Chip
              label={row.original.warehouseStatus!.toString().charAt(0).toUpperCase() + row.original.warehouseStatus!.toString().slice(1)}
              variant='tonal'
              color={row.original.warehouseStatus === 'available' ? 'info' : 'error'}
              size='small'
            />
            {
              row.original.warehouseRequisitionRequest! > 0 && (
            <Chip
  ...
  const table = useReactTable({
    data: data as ProductionCategoryType[],
    columns,
    filterFns: {
      fuzzy: fuzzyFilter
    },
  ...
  return (
    <>
      <Typography className='mt-4' variant='h5'>Make Product Requisition From Production</Typography>
...
export default WarehouseSalesRequisitionTable
