...
// Type Imports
import type { ProductionRequisition } from '@/types/apps/ecommerceTypes'
...
import { getWarehouseRequisitionList } from '@/libs/actions/warehouse.actions'
import { approveWarehouseRequisition, denyWarehouseRequisition } from '@/libs/actions/sales.actions'
...
const WarehouseSalesRequisitionTable = () => {
  // States
  const user = useAuth()
  ...
  const fetchProductionData = useCallback(async () => {
    try {
      setIsLoading(true)
      const response = await getWarehouseRequisitionList()
      setAllData(response?.documents as unknown as ProductionRequisition[])
      setData(response?.documents as unknown as ProductionRequisition[])
    } catch (error) {
      console.error('Error fetching warehouse requisition data:', error)
  ...
  const handleBulkAction = async (action: 'approve' | 'deny') => {
    try {
      ...
      for (const row of selectedRows) {
        if (action === 'approve') {
          await approveWarehouseRequisition(row.$id!, row.category.$id, user.user.$id)
        } else {
          await denyWarehouseRequisition(row.$id!, row.category.$id, user.user.$id)
        }
      }
  ...
  const handleRequisitionAction = async (
    ...
    try {
      setIsLoading(true)
      if (action === 'approve') {
        await approveWarehouseRequisition(requisitionId, categoryId, user.user.$id)
        toast.success('Requisition approved successfully')
      } else {
        await denyWarehouseRequisition(requisitionId, categoryId, user.user.$id)
  ...
  return (
    <>
      <Typography className='mt-4' variant='h4'>Requisitions Made From Warehouse To Sales</Typography>
