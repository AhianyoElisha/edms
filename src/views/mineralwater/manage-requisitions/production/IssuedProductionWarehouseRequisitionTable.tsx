...
import { issueStoreRequisition } from '@/libs/actions/stores.actions'
import { formatDate, formatTime } from '@/utils/dateAndCurrency'
import { toast } from 'react-toastify'
import LoaderDark from '@/components/layout/shared/LoaderDark'
import { getIssuedProductionRequisitionList } from '@/libs/actions/production.actions'
import { issueProductionRequisition } from '@/libs/actions/warehouse.actions'
...
const ProductionWarehouseRequisitionTable = () => {
  ...
  const fetchProductionData = useCallback(async () => {
    try {
      setIsLoading(true)
      const response = await getIssuedProductionRequisitionList()
  ...
  const handleBulkAction = async (action: 'issue' | 'deny') => {
    try {
      ...
      for (const row of selectedRows) {
        if (action === 'issue') {
          await issueProductionRequisition(row.$id!, row.category.$id, user.user.$id)
        }
      }
  ...
  const handleRequisitionAction = async (
    ...
    try {
      setIsLoading(true)
      if (action === 'issue') {
        await issueProductionRequisition(requisitionId, categoryId, user.user.$id)
  ...
  return (
    <>
      <Typography className='mt-4' variant='h4'>Issue Approved Items to Warehouse</Typography>
