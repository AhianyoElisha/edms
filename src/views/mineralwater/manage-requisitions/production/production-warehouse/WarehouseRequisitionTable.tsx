...
const WarehouseRequisitionTable = ({ refreshTrigger, onSuccessfulSubmit }: ProductionStoreRequisitionTableProps) => {
  // States
  ...
  const columns = useMemo<ColumnDef<ProductionCategoryWithActionsType, any>[]>(
    () => [
      ...
      //             try {
      //               approveProductionRequisition(row.original.$id, row.original.title, user.user.$id);
      ...
      //             onClick: () => {
      //               try {
      //                 denyProductionRequisition(row.original.$id, row.original.title, user.user.$id);
  ...
  return (
    <>
      <Typography className='mt-4' variant='h5'>Push Products From Production To Warehouse</Typography>
...
export default WarehouseRequisitionTable
