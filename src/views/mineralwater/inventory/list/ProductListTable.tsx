...
import { getInventoryCategoryList, getInventoryList } from '@/libs/actions/stores.actions'
...
type ProductWithActionsType = InventoryListType & {
  actions?: string
}
...
const ProductListTable = () => {
  ...
  const [data, setData] = useState<InventoryListType[]>([])
  const [filteredData, setFilteredData] = useState(data)
  const [globalFilter, setGlobalFilter] = useState<InventoryListType['productBrand']>('')
  ...
  const fetchInventoryData = useCallback(async () => {
    try {
      ...
      setData(response?.documents as unknown as InventoryListType[])
      const category = await getInventoryCategoryList(false, true)
      setCategoryData(category?.inventoryList?.documents as unknown as CategoryType[])
    } catch (error) {
    ...
    () => [
      ...
      columnHelper.accessor('usdRate', {
        ...
        cell: ({ row }) => (
          <div className='flex items-center gap-3'>
            {/* @ts-ignore */}
            <Typography color='text.primary'>{row.original.usdRate}</Typography>
          </div>
        )
      }),
          ...
          <div className='flex items-center'>
            <OptionMenu
              ...
              options={[
                { text: 'Details', 
                  icon: 'ri-stack-line',
                  menuItemProps: { onClick: () => router.push(`/stores/${row.original.$id}`) }
                },
  ...
  const table = useReactTable({
    data: filteredData as InventoryListType[],
    columns,
    filterFns: {
      fuzzy: fuzzyFilter
    },
  ...
  return (
    <>
      <Typography className='mt-4' variant='h4'>Inventory Report List</Typography>
      <Divider className='my-8' />
      <Card>
        <CardHeader title='Filters' />
        <TableFilters setData={setFilteredData} productData={data} categoryData={categoryData as unknown as CategoryType[]} />
        <Divider />
