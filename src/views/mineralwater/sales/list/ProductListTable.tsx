...
import type { InventoryListType, ManufacturedProductListType, ProductDetailsType, ProductionCategoryType } from '@/types/apps/ecommerceTypes'
...
import { getWarehouseProductList } from '@/libs/actions/warehouse.actions'
import WarehouseExportDialog from '@/components/dialogs/warehouse-dialog'
import { getSalesProductList } from '@/libs/actions/sales.actions'
...
type ProductWithActionsType = ManufacturedProductListType & {
  actions?: string
}
...
const ProductListTable = () => {
  ...
  const [data, setData] = useState<ManufacturedProductListType[]>([])
  const [filteredData, setFilteredData] = useState(data)
  const [globalFilter, setGlobalFilter] = useState<ProductionCategoryType['title']>('')
  const [globalFilteredData, setGlobalFilteredData] = useState<InventoryListType[]>([])
  ...
  const fetchInventoryData = useCallback(async () => {
    try {
      ...
      setData(response?.documents as unknown as ManufacturedProductListType[])
          ...
          <div className='flex items-center gap-3'>
            ...
            <div className='flex flex-col items-start'>
              <Typography className='font-medium' color='text.primary'>
                {
                //@ts-ignore
                  row.original.productCategory.title
                }
              </Typography>
          ...
          <div className='flex items-center'>
            <OptionMenu
              ...
              options={[
                { text: 'Details', 
                  icon: 'ri-stack-line',
                  menuItemProps: { onClick: () => router.push(`/production/${row.original.$id}`) }
                },
  ...
  const table = useReactTable({
    data: filteredData as ManufacturedProductListType[],
    columns,
    filterFns: {
      fuzzy: fuzzyFilter
    },
  ...
  return (
    <>
      <Typography className='mt-4' variant='h3'>Sales Console Products Report</Typography>
      <Divider className='my-8' />
      <Card>
        <CardHeader title='Filters' />
        <TableFilters setData={setFilteredData} productData={data} categoryData={categoryData} />
        <Divider />
        <div className='flex flex-col justify-end items-end sm:flex-row sm:items-center gap-y-4 p-5'>
          {/* <DebouncedInput
            value={globalFilter ?? ''}
