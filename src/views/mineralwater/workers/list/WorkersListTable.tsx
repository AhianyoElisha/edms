...
import type { Customer, Worker } from '@/types/apps/ecommerceTypes'
...
import { getWorkersList } from '@/libs/actions/customer.action'
...
type ECommerceOrderTypeWithAction = Worker & {
  actions?: string
}
...
const WorkersListTable = () => {
  // States
  const [workersUserOpen, setWorkersUserOpen] = useState(false)
  ...
  const fetchWorkersData = useCallback(async () => {
    try {
      ...
      setData(response?.documents as unknown as Worker[])
    } catch (error) {
  ...
  const columns = useMemo<ColumnDef<ECommerceOrderTypeWithAction, any>[]>(
    () => [
        ...
        cell: ({ row }) => (
          <div className='flex items-center gap-3'>
            ...
            <div className='flex flex-col items-start'>
              {/* <Link href={`/worker/${row.original.$id}`}> */}
                <Typography
                  color='text.primary'
      ...
      //           { text: 'Details', 
      //             icon: 'ri-stack-line',
      //             menuItemProps: { onClick: () => router.push(`/workers/${row.original.$id}`) }
      //           },
      //         ]}
      //       />
      //     </div>
  ...
  return (
    <>
      <Typography className='mt-4' variant='h4'>Workers List</Typography>
        ...
        <CardContent className='flex justify-between flex-wrap max-sm:flex-col sm:items-center gap-4'>
          ...
          <div className='flex gap-4 max-sm:flex-col max-sm:is-full'>
            ...
            <Button
              ...
              onClick={() => setWorkersUserOpen(!workersUserOpen)}
            >
              Add Workers
            </Button>
          </div>
      ...
      <AddWorkersDrawer
        open={workersUserOpen}
        handleClose={() => setWorkersUserOpen(!workersUserOpen)}
        workerData={data}
        onSuccess={fetchWorkersData}
      />
    </>
  )
}
