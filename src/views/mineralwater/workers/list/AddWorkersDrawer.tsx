...
// Types
import type { Worker } from '@/types/apps/ecommerceTypes'
import { saveWorkersToDB } from '@/libs/actions/customer.action'
...
const formSchema = z.object({
  name: z.string().min(1, { message: "Worker is required" }),
  contact: z.string().min(1, { message: "Contact is required" }),
  address: z.string().min(1, { message: "Address Line  is required" }),
  workarea: z.string().min(1, { message: "Work area is required" }),
})


const AddWorkersDrawer = (props: Props) => {
  ...
  const onSubmit = async (data: Worker) => {
    try {
      ...
      const res = await saveWorkersToDB(data)
  ...
  return (
    ...
    >
      <div className='flex items-center justify-between p-5'>
        <Typography variant='h5'>Add a Worker</Typography>
            ...
            <Controller
              ...
              render={({ field }) => (
                <TextField
                  ...
                  helperText={errors.workarea?.message}
                />
              )}
            />
...
export default AddWorkersDrawer