...
import type { DistributedItemDetailType, InventoryListType, ManufacturedProductListType, ProductDetailsType, ProductionCategoryType, ReturnItemDetailType, TransactionItemDetailType } from '@/types/apps/ecommerceTypes'
...
import { getReturnedProductList } from '@/libs/actions/warehouse.actions'
...
type TransactionWithActionsType = ReturnItemDetailType & {
  actions?: string
}
...
const TransactionListTable = () => {
  ...
  const [data, setData] = useState<ReturnItemDetailType[]>([])
  ...
  const fetchInventoryData = useCallback(async () => {
    try {
      setIsLoading(true)
      const response = await getReturnedProductList()
      setData(response?.documents as unknown as ReturnItemDetailType[])
  ...
  const columns = useMemo<ColumnDef<TransactionWithActionsType, any>[]>(
    () => [
      ...
      columnHelper.accessor('returnDate', {
        header: 'Return Date',
        cell: ({ row }) => {
          const dateString = row.original.returnDate!;
          let formattedDate;
          ...
          <div className='flex items-center'>
            <OptionMenu
              ...
              options={[
                { text: 'Details', 
                  icon: 'ri-stack-line',
                  menuItemProps: { onClick: () => router.push(`/returns/${row.original.$id}`) }
                },
  ...
  const table = useReactTable({
    data: filteredData as ReturnItemDetailType[],
    columns,
    filterFns: {
      fuzzy: fuzzyFilter
    },
  ...
  return (
    <>
      <Typography className='mt-4' variant='h3'>All Goods Returned Report</Typography>
      <Divider className='my-8' />
      <Card>
        <CardHeader title='Filters' />
        <TableFilters setData={setFilteredData} returnedProductsData={data} categoryData={categoryData}/>
        <Divider />
