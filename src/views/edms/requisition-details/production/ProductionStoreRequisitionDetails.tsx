...
import {  ManufacturedProductDetailsType, ProductionCategoryType } from '@/types/apps/ecommerceTypes'

import { formatPrice } from '@/utils/dateAndCurrency'



const ProductionDetails = ({ data }: { data?: ManufacturedProductDetailsType }) => {
  ...
  return (
    <div className='flex flex-col gap-10'>
      <Typography variant='h5'>{`Stores -> Production Requisition`}</Typography>
        ...
        <CardContent className='flex flex-wrap items-center justify-between gap-4 pbe-6'>
          <div>
            <Typography variant='h5'>{
              // @ts-ignore
              data?.productCategory.title ?? ''}</Typography>
          </div>
        </CardContent>
        <CardContent>
          <div className='border rounded'>
            ...
            <div className='flex flex-col gap-6 p-5'>
                ...
                <div className='flex flex-wrap gap-x-12 gap-y-2'>
                  <List role='list' component='div' className='flex flex-col gap-2 plb-0'>
                    <ListItem role='listitem' className='flex items-center gap-2 p-0'>
                      ...
                      <Typography>Number of Packages Available for Requisition: {
                        // @ts-ignore
                        data?.productCategory.totalProducts}</Typography>
                    </ListItem>
                    <ListItem role='listitem' className='flex items-center gap-2 p-0'>
                      <i className='ri-dropbox-line text-xl text-textSecondary' />
                      {/* @ts-ignore */}
                      <Typography>Quantity Per Box: {data?.qtyPerPackage}</Typography>
                    </ListItem>
                    <ListItem role='listitem' className='flex items-center gap-2 p-0'>
                  ...
                  <List role='list' component='div' className='flex flex-col gap-2 plb-0'>
                    ...
                    <ListItem role='listitem' className='flex items-center gap-2 p-0'>
                      <i className='ri-dropbox-line text-xl text-textSecondary' />
                      <Typography>Quantity Requested: {
                        // @ts-ignore
                        data?.productCategory.requisitionRequest}</Typography>
                    </ListItem>
                  </List>
                </div>
              </div>
              ...
              <div className='flex flex-col gap-4'>
                ...
                <Typography>{
                // @ts-ignore
                  data?.productCategory.description}</Typography>
              </div>
              {/* <Divider /> */}
              {/* <div className='flex flex-col gap-4'>
