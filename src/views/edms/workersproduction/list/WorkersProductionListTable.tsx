...
import type { DistributedItemDetailType, InventoryListType, ManufacturedProductListType, ProductDetailsType, ProductionCategoryType, TransactionItemDetailType, WorkerDetailType } from '@/types/apps/ecommerceTypes'
...
import { getManufacturedProductList, getProductionCategoryList, getWorkersProductionList } from '@/libs/actions/production.actions'
...
type TransactionWithActionsType = WorkerDetailType & {
  actions?: string
}
...
const TransactionListTable = () => {
  ...
  const [data, setData] = useState<WorkerDetailType[]>([])
  ...
  const fetchWorkersProductionData = useCallback(async () => {
    try {
      setIsLoading(true)
      const response = await getWorkersProductionList()
      console.log('Transaction data fetched successfully:', response)
      setData(response?.documents as unknown as WorkerDetailType[])
  ...
  const columns = useMemo<ColumnDef<TransactionWithActionsType, any>[]>(
    () => [
      ...
      columnHelper.accessor('productionDate', {
        ...
        cell: ({ row }) => {
          const dateString = row.original.productionDate!;
          let formattedDate;
  ...
  const table = useReactTable({
    data: filteredData as WorkerDetailType[],
    columns,
    filterFns: {
      fuzzy: fuzzyFilter
    },
  ...
  return (
    <>
      <Typography className='mt-4' variant='h3'>All Worker's Production Report</Typography>
      <Divider className='my-8' />
      <Card>
        <CardHeader title='Filters' />
        <TableFilters setData={setFilteredData} workerProductionData={data} categoryData={categoryData}/>
        <Divider />
        <div className='flex justify-end flex-col items-end sm:flex-row sm:items-center gap-y-4 p-5'>
          <div className='flex items-center max-sm:flex-col gap-4 max-sm:is-full is-auto'>
            ...
            <Button
              variant='contained'
              component={Link}
              href={'/workersproduction'}
              startIcon={<i className='ri-add-line' />}
              className='max-sm:is-full is-auto'
            >
              Add Worker's Production
            </Button>
          </div>
        </div>
