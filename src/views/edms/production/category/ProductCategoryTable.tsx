...
import ProductionCategoryExportDialog from '@/components/dialogs/production-dialog/category'
...
type CategoryWithActionsType = ProductionCategoryType & {
  actions?: string
}
...
const ProductCategoryTable = () => {
  ...
  const [editCategoryOpen, setEditCategoryOpen] = useState<{ open: boolean; data: ProductionCategoryType | null }>({ open: false, data: null })
  const [rowSelection, setRowSelection] = useState({})
  const [data, setData] = useState<ProductionCategoryType[]>([])
  ...
  const fetchProductionData = useCallback(async () => {
    try {
      setIsLoading(true)
      const response = await getProductionCategoryList(false, false)
      setData(response?.productionList?.documents as unknown as ProductionCategoryType[])
    } catch (error) {
      console.error('Error fetching manufactured products data:', error)
      toast.error('Failed to fetch manufactured products data')
    } finally {
      setIsLoading(false)
    }
  }, [])
  ...
  const columns = useMemo<ColumnDef<CategoryWithActionsType, any>[]>(
    () => [
      ...
      columnHelper.accessor('qtyPerPackage', {
        header: 'Qty/Package',
        cell: ({ row }) => (
          <Typography>
            {
              row.original.qtyPerPackage
            }
          </Typography>
        )
      }),
  ...
  return (
    <>
      <Typography className='mt-4' variant='h4'>Production Categories</Typography>
        ...
        <div className='flex items-start justify-between max-sm:flex-col sm:items-center gap-y-4 p-5'>
          ...
          <div className='flex items-center max-sm:flex-col gap-4 max-sm:is-full'>
            <OpenDialogOnElementClick 
              ...
              dialog={ProductionCategoryExportDialog} 
