...
// Type Imports
import type { CategoryType, InventoryListType, StoreRequisition } from '@/types/apps/ecommerceTypes'
...
import { approveStoreRequisition, denyStoreRequisition, getStoreRequisitionList } from '@/libs/actions/stores.actions'
...
type CategoryWithActionsType = StoreRequisition & {
  actions?: string
}
...
const StoreProductionRequisitionTable = () => {
  // States
  const user = useAuth()
  const [rowSelection, setRowSelection] = useState({})
  const [allData, setAllData] = useState<StoreRequisition[]>([])
  const [data, setData] = useState<StoreRequisition[]>([])
  const [globalFilter, setGlobalFilter] = useState('')
  const [isLoading, setIsLoading] = useState(true)
  const [editCategoryOpen, setEditCategoryOpen] = useState<{ open: boolean; data: StoreRequisition | null }>({ open: false, data: null })
  ...
  const fetchRequisitionData = useCallback(async () => {
    try {
      setIsLoading(true)
      const response = await getStoreRequisitionList()
      setAllData(response?.documents as unknown as StoreRequisition[])
      setData(response?.documents as unknown as StoreRequisition[])
    } catch (error) {
  ...
  const handleBulkAction = async (action: 'approve' | 'deny') => {
    try {
      ...
      for (const row of selectedRows) {
        if (action === 'approve') {
          await approveStoreRequisition(row.$id!, row.category.$id, user.user.$id)
        } else {
          await denyStoreRequisition(row.$id!, row.category.$id, user.user.$id)
        }
      }
  ...
  const handleRequisitionAction = async (
    ...
    try {
      setIsLoading(true)
      if (action === 'approve') {
        await approveStoreRequisition(requisitionId, categoryId, user.user.$id)
        toast.success('Requisition approved successfully')
      } else {
        await denyStoreRequisition(requisitionId, categoryId, user.user.$id)
  ...
  const columns = useMemo<ColumnDef<CategoryWithActionsType, any>[]>(() => {
    const baseColumns: ColumnDef<CategoryWithActionsType, any>[] = []
    ...
    baseColumns.push(
      columnHelper.accessor('category.categoryTitle', {
          ...
          <div className='flex items-center gap-3'>
            ...
            <div className='flex flex-col items-start'>
              <Typography className='font-medium' color='text.primary'>
                {row.original.category.categoryTitle}
              </Typography>
  ...
  return (
    <>
      <Typography className='mt-4' variant='h4'>Requisitions Made From Production</Typography>
      ...
      <EditCategoryDrawer
        ...
        requisitionData={editCategoryOpen.data!}
        handleClose={() => setEditCategoryOpen({ open: false, data: null })}
        onSuccess={fetchRequisitionData}
      />
    </>
  )
}

export default StoreProductionRequisitionTable
