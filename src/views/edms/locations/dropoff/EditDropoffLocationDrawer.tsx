...
// Types
import { DropoffLocationType, GPSCoordinates } from '@/types/apps/deliveryTypes'
import { updateDropoffLocation } from '@/libs/actions/location.actions'
import { WEST_AFRICAN_COUNTRIES } from '@/data/countries'

interface EditDropoffLocationDrawerProps {
  open: boolean
  location: DropoffLocationType | null
  onClose: () => void
  onSuccess: () => void
}

const EditDropoffLocationDrawer: React.FC<EditDropoffLocationDrawerProps> = ({
  open,
  location,
  onClose,
  onSuccess
  ...
  const handleLocationSelect = (coordinates: GPSCoordinates, address?: string, placeDetails?: any) => {
    ...
    const locationCode = `DO-${coordinates.latitude.toFixed(4)}-${coordinates.longitude.toFixed(4)}`
...
export default EditDropoffLocationDrawer
