...
import { PickupLocationType } from '@/types/apps/deliveryTypes'
import { 
...
import AddPickupLocationDrawer from './AddPickupLocationDrawer'
import EditPickupLocationDrawer from './EditPickupLocationDrawer'
...
const PickupLocationOverviewTable: React.FC<PickupLocationOverviewTableProps> = ({
  onEditLocation,
  onAddLocation
}) => {
  // State management
  ...
  const [selectedLocation, setSelectedLocation] = useState<PickupLocationType | null>(null)
  const [viewDialogOpen, setViewDialogOpen] = useState(false)
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false)
  const [locationToDelete, setLocationToDelete] = useState<PickupLocationType | null>(null)
  ...
  const [locationToEdit, setLocationToEdit] = useState<PickupLocationType | null>(null)
  ...
  const fetchLocations = async () => {
    try {
      ...
      setStatistics(statsData.pickup)
    } catch (error) {
  ...
  const handleViewLocation = (location: PickupLocationType) => {
    setSelectedLocation(location)
    setViewDialogOpen(true)
  }

  const handleEditLocation = (location: PickupLocationType) => {
    setLocationToEdit(location)
    setEditDrawerOpen(true)
  }

  const handleDeleteClick = (location: PickupLocationType) => {
  ...
  const handleConfirmDelete = async () => {
    if (locationToDelete) {
      try {
        await deletePickupLocation(locationToDelete.$id)
  ...
  const handleToggleStatus = async (location: PickupLocationType) => {
    try {
  ...
  return (
    <>
                      ...
                      <OptionMenu
                        ...
                        options={[
                          ...
                          },
                          {
                            ...
                            icon: location.isActive ? 'ri-pause-circle-line' : 'ri-play-circle-line',
                            linkProps: { className: 'flex items-center gap-2 is-full plb-1.5 pli-4' },
      ...
      {/* Edit Pickup Location Drawer */}
      <EditPickupLocationDrawer
        open={editDrawerOpen}
...
export default PickupLocationOverviewTable