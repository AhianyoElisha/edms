...
const RateCardEditForm = ({ rateCard, userId }: RateCardEditFormProps) => {
  ...
  // Create initial volume price map, keyed by the tier the stored rate belongs to
  const initialPriceMap = createEmptyVolumePriceMap()
  VOLUME_TIERS.forEach(tier => {
    initialPriceMap[tier.volume] = findVolumePriceForTier(existingVolumePrices, tier)?.rate || 0
  })
