...
type CategoryWithActionsType = SalesCategoryType & {
  actions?: string
}
...
const SalesCategoryTable = () => {
  // States
  const [addCategoryOpen, setAddCategoryOpen] = useState(false)
  const [editCategoryOpen, setEditCategoryOpen] = useState<{ open: boolean; data: SalesCategoryType | null }>({ open: false, data: null })
  ...
  const fetchSalesCategoryData = useCallback(async () => {
    try {
      setIsLoading(true)
      const response = await getSalesCategoryList()
      setData(response?.documents as unknown as CategoryType[])
    } catch (error) {
  ...
  return (
    <>
      ...
      <AddCategoryDrawer
        ...
        onSuccess={fetchSalesCategoryData}
      />
      <EditCategoryDrawer
