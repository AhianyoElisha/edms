...
const DistributionExportDialog = ({ open, setOpen, data, tableData }: ExportDialogProps & { tableData: any[] }) => {
...
const exportToPDF = (data: any[]) => {
  ...
  img.onload = () => {
    ...
    doc.setFontSize(14)
    doc.setFont('helvetica', 'bold')
    const title = 'Distributed Products Report'
    ...
    // Create table data
    const tableColumn = ['Category', 'Created Date', 'Quantity', 'Total Price(GH₵)']
    ...
    // Save the PDF
    doc.save(`distribution report. ${new Date().toISOString().split('T')[0]} ${new Date().toLocaleTimeString()}.pdf`)
  }
}
  ...
  const companyDetails = [
    ...
    ['', '', 'Distributed Products Report', '', `Time Generated: ${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString()}`],
    [], // Empty row for spacing
    ['Category', 'Date Created', 'QTY(Boxes)', 'Total Price(GH₵)']
  ]
...
export default DistributionExportDialog
