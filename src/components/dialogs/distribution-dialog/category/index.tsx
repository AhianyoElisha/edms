...
const ProductionCategoryExportDialog = ({ open, setOpen, data, tableData }: ExportDialogProps & { tableData: any[] }) => {
...
const exportToPDF = (data: any[]) => {
  ...
  img.onload = () => {
    ...
    const address = [
      ...
      'Email: flowmineralwater@gmail.com'
    ]
    ...
    doc.setFontSize(14)
    doc.setFont('helvetica', 'bold')
    const title = 'Manufacturing Categories Report'
    ...
    // Create table data
    const tableColumn = ['Category', 'Created Date', 'QTY/Package', 'Total Products', 'Price/Package']
    ...
    // Save the PDF
    doc.save(`production category report. ${new Date().toISOString().split('T')[0]} ${new Date().toLocaleTimeString()}.pdf`)
  }
}
  ...
  const companyDetails = [
    ...
    ['', '', 'Manufacturing Categories Report', '', `Time Generated: ${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString()}`],
    [], // Empty row for spacing
    ['Category', 'Created Date', 'QTY/Package', 'Total Products', 'Price/Package']
  ]
...
export default ProductionCategoryExportDialog
