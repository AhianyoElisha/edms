...
const InventoryCategoryExportDialog = ({ open, setOpen, tableData }: InventoryDialogProps & { tableData: any[] }) => {
...
const exportToPDF = (data: any[]) => {
  ...
  img.onload = () => {
    ...
    doc.setFontSize(14)
    doc.setFont('helvetica', 'bold')
    const title = 'Inventory Stock Availability Report'
    ...
    // Create table data
    const tableColumn = ['Category', 'Stock Limit', 'Total Products', 'Status']
    ...
    printFrame.onload = () => {
      try {
        const handlePrintComplete = () => {
          ...
          const fileName = `inventory category report ${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString()}.pdf`
...
const exportToExcel = (data: any[]) => {
  ...
  const companyDetails = [
    ...
    ['', '', 'Inventory Stock Availability Report', '', `Time Generated: ${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString()}`],
    [], // Empty row for spacing
    ['Category', 'Stock Limit', 'Total Products', 'Status']
  ]
  ...
  // Save the file
  XLSX.writeFile(wb, `inventory stock report ${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString()}.xlsx`)
}
...
export default InventoryCategoryExportDialog
