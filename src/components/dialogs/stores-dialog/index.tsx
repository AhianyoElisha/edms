...
const InventoryExportDialog = ({ open, setOpen, tableData }: InventoryDialogProps & { tableData: any[] }) => {
...
const exportToPDF = (data: any[]) => {
  ...
  img.onload = () => {
    ...
    doc.setFontSize(14)
    doc.setFont('helvetica', 'bold')
    const title = 'Inventory Purchase Report'
    ...
    const titleX = (pageWidth - titleWidth - dateWidth - 20) / 2  // 20 is spacing between title and date
    ...
    // Create table data
    const tableColumn = ['Product Company', 'Category', 'Date Purchased', 'Total Price(GH₵)', 'QTY(Boxes)', 'Status']
    ...
    printFrame.onload = () => {
      try {
        const handlePrintComplete = () => {
          ...
          const fileName = `inventory report ${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString()}.pdf`
...
const exportToExcel = (data: any[]) => {
  ...
  const companyDetails = [
    ...
    ['', '', 'Inventory Purchase Report', '', `Time Generated: ${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString()}`],
    [], // Empty row for spacing
    ['Product Company', 'Category', 'Date Purchased', 'Total Price(GH₵)', 'QTY(Boxes)', 'Status']
  ]
  ...
  // Save the file
  XLSX.writeFile(wb, `inventory report ${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString()}.xlsx`)
}
