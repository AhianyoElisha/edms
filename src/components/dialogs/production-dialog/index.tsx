...
const ProductionExportDialog = ({ open, setOpen, data, tableData }: ExportDialogProps & { tableData: any[] }) => {
...
const exportToPDF = (data: any[]) => {
  ...
  img.onload = () => {
    ...
    doc.setFontSize(14)
    doc.setFont('helvetica', 'bold')
    const title = 'Manufactured Products Report'
    ...
    // Create table data
    const tableColumn = ['Category', 'Total Price(GH₵)', 'Created Date', 'Quantity', 'Status']
    ...
    printFrame.onload = () => {
      try {
        const handlePrintComplete = () => {
          ...
          const fileName = `production report ${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString()}.pdf`
  ...
  const companyDetails = [
    ...
    ['', '', 'Manufactured Products Report', '', `Time Generated: ${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString()}`],
    [], // Empty row for spacing
    ['Category', 'Date Purchased', 'Total Price(GH₵)', 'QTY(Boxes)', 'Status']
  ]
...
export default ProductionExportDialog
