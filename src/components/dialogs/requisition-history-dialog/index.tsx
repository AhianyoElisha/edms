...
type RequisitionHistoryDialogProps = {
  open: boolean
...
const RequisitionHistoryExportDialog = ({ open, setOpen, tableData }: RequisitionHistoryDialogProps & { tableData: any[] }) => {
...
const exportToPDF = (data: any[]) => {
  ...
  img.onload = () => {
    ...
    doc.setFontSize(14)
    doc.setFont('helvetica', 'bold')
    const title = 'Requisition History Report'
    const date = `Time Generated: ${new Date().toLocaleDateString()}, ${new Date().toLocaleTimeString()}`
    const titleWidth = doc.getStringUnitWidth(title) * doc.getFontSize() / doc.internal.scaleFactor
    const dateWidth = doc.getStringUnitWidth(date) * doc.getFontSize() / doc.internal.scaleFactor
    const titleX = (pageWidth - titleWidth - dateWidth - 1) / 2  // 20 is spacing between title and date
    doc.text(title, titleX, yPosition + 10)
    doc.setFontSize(11)
    doc.text(date, titleX + titleWidth + 10, yPosition + 10)
    ...
    // Create table data
    const tableColumn = ['Category of Requisition', 'Action Performed', 'Date']
    const tableRows = data.map(item => [
      item.category,
      item.requisitionist.name + ' ' + item.requisitionEvent + ' ' + item.noOfBoxes + ' boxes or packages from ' + item.requisitionType,
    ...
    printFrame.onload = () => {
      try {
        const handlePrintComplete = () => {
          ...
          const fileName = `requisition history report ${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString()}.pdf`
...
const exportToExcel = (data: any[]) => {
  ...
  const companyDetails = [
    ...
    ['', '', 'Requisition History Report', '', `Time Generated: ${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString()}`],
    [], // Empty row for spacing
    ['Category', 'Action Performed', 'Date']
  ]
  ...
  const columnWidths = [
    { wch: 20 }, // Category
    { wch: 100 }, // Action Performed
    { wch: 20 }, // Date Purchased
  ]
  ...
  // Save the file
  XLSX.writeFile(wb, `requisition history report ${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString()}.xlsx`)
}
...
export default RequisitionHistoryExportDialog
